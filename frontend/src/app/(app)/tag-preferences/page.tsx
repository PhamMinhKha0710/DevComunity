...
export default function TagPreferencesPage() {
    ...
    const { data: tagsData, isLoading: tagsLoading } = useQuery({
        queryKey: ['tags', { pageSize: 100 }],
        queryFn: () => tagsApi.list({ pageSize: 100 }),
        enabled: !!user,
    });

    const { data: prefsData, isLoading: prefsLoading } = useQuery({
        queryKey: ['userTagPreferences'],
        queryFn: () => apiClient.get('/users/tag-preferences').then(r => r.data),
        enabled: !!user,
    });
    ...
    useEffect(() => {
        if (prefsData) {
            setWatchedTags(prefsData.watched || []);
            setIgnoredTags(prefsData.ignored || []);
        }
    }, [prefsData]);
    ...
    const saveMutation = useMutation({
        mutationFn: (data: { watched: string[]; ignored: string[] }) =>
            apiClient.put('/users/tag-preferences', data),
    });

    const savePreferences = () => {
        saveMutation.mutate({ watched: watchedTags, ignored: ignoredTags });
    };
